...
class HomePage {
  ...
  async loadLatestPublications() {
    ...
    try {
      ...
      this.renderError(container, 'Error loading publications');
    }
  }

  renderPublications(container, publications) {
    if (publications.length === 0) {
      container.innerHTML = '<p class="text-center text-muted">No publications available</p>';
      return;
    }
      ...
      <article class="publication-card">
        ...
        <div class="publication-card__footer">
          ...
          <div class="publication-card__links">
            ${pub.doi ? `<a href="https://doi.org/${pub.doi}" class="publication-card__link" target="_blank" rel="noopener">DOI</a>` : ''}
            ${pub.pdf_url ? `<a href="${pub.pdf_url}" class="publication-card__link" target="_blank" rel="noopener">PDF</a>` : ''}
          </div>
        </div>
      </article>
    `).join('');
  ...
  async loadFeaturedProjects() {
    ...
    try {
      ...
      this.renderError(container, 'Error loading projects');
    }
  }
  ...
  renderProjects(container, projects) {
    ...
    const html = projects.map(project => `
      <article class="project-card">
        ${project.image_url ? `<img src="${project.image_url}" alt="${this.escapeHtml(project.title)}" class="project-card__image" loading="lazy">` : ''}
        <div class="project-card__content">
          ...
          <div class="project-card__footer">
            ...
            <div class="project-card__links">
              ${project.repository_url ? `<a href="${project.repository_url}" class="btn btn--sm btn--outline" target="_blank" rel="noopener">Repository</a>` : ''}
            </div>
          </div>
        </div>
      </article>
  ...
  async loadLatestNews() {
    ...
    try {
      ...
      this.renderError(container, 'Error loading news');
    }
  }
  ...
  renderNews(container, newsItems) {
    ...
    const html = newsItems.map(item => `
      <article class="news-card">
        ${item.image_url ? `<img src="${item.image_url}" alt="${this.escapeHtml(item.title)}" class="news-card__image" loading="lazy">` : ''}
